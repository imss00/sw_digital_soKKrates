...
4) photo: Vision LABEL_DETECTION 결과가 있을 때만 장면 키워드로 사용한다.
"""
import json
import re
...
def build_photo_section(user_id: int, target_date: date, db: Session) -> dict:
    """Vision LABEL_DETECTION 결과가 있는 사진만 장면 grounding으로 사용한다."""
    ...
    photos = (
        ...
        .filter(
            ...
            Photo.vision_labels.isnot(None),
        )
    ...
    photo_labels: list[dict] = []
    for photo in photos:
        try:
            vision = json.loads(photo.vision_labels or "{}")
        ...
        if photo.latitude is not None and photo.longitude is not None:
            location = f"{photo.latitude:.4f}, {photo.longitude:.4f}"
        photo_labels.append({
            "labels": labels[:8],
            "location": location,
            "taken_at": photo.taken_at.astimezone(KST).strftime("%H:%M") if photo.taken_at else "",
        })

    if label_counter:
        keywords = [label for label, _ in label_counter.most_common(8)]
        return {
            "_available": True,
            "_source": "photos.vision_labels",
            "photo_keywords": keywords,
            "photo_mood": None,
            "recommend_category": keywords[0],
    ...
    return {
        "_available": False,
        "_source": "none",
        "_reason": "사진 장면 라벨(LABEL_DETECTION) 없음 — 키워드 grounding 불가",
        "photo_keywords": [],
        "photo_mood": None,
