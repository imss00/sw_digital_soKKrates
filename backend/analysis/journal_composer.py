...
- A5 양면 기준: 각 섹션 글자수 충분히 확보
    ...
    · 회고 / 기사소개 / 음악: 8~10문장, 400~550자 내외 (본문 분량 섹션)
...
# ────────────────────────────────────────────────────────────────
# B-4. 어제 회고
# (본문 섹션 — 8~10문장, 400~550자로 확장)
# ────────────────────────────────────────────────────────────────

_REFLECTION_VARIANTS = [
    # variant A: 시간 흐름 서술 — 아침→저녁
    _wrap(
        """다음 데이터를 바탕으로 어제 하루를 8~10문장(400~550자)으로 회고해주세요.
시간 흐름(아침→낮→저녁)을 자연스럽게 따라가며, "~했다" 체로 담담하게 서술하세요.
평가나 조언 없이 기록하듯 쓰고, 구체적인 키워드나 활동을 최소 4개 포함하세요.
...
어제의 장면: {photo_narrative}"""
    ...
    _wrap(
        """다음 데이터를 바탕으로 어제 하루를 8~10문장(400~550자)으로 회고해주세요.
하루를 관통한 키워드 2~3개를 각각 한두 문장씩 풀어 쓰며, 무엇에 시간을 쏟았는지를
"~에 빠져 있었다", "~를 붙잡고 있었다" 같은 표현으로 서술하세요.
중간에는 그 활동을 하며 들었을 법한 감상이나 생각을 한두 문장 덧붙이세요.
마지막 한두 문장은 하루 전체를 압축하는 마무리로 끝내세요.
...
어제의 장면: {photo_narrative}"""
    ...
    _wrap(
        """다음 데이터를 바탕으로 어제 하루를 8~10문장(400~550자)으로 회고해주세요.
청각(음악), 시각(영상/사진), 사고(관심사)의 세 감각을 각각 두세 문장씩 담아 서술하세요.
"~를 들으며", "~를 보며", "~를 생각하며" 같은 연결어를 활용해 자연스럽게 이어주고,
각 감각 사이에는 짧은 전환 문장을 하나씩 넣어 흐름이 끊기지 않게 하세요.
...
사용자의 관심도가 가장 높을 것으로 예상되는 기사 1개를 메인 기사로 설정하세요.
메인 기사는 8~10문장(450~550자)으로 작성하세요.
다른 기사들은 기사당 6~8문장(350~450자)으로, 사용자의 관심사와 어떻게 연결되는지를
먼저 짚은 뒤 기사 핵심 내용을 풀어서 소개하세요.
...
사용자의 관심도가 가장 높을 것으로 예상되는 기사 1개를 메인 기사로 설정하세요.
메인 기사는 8~10문장(450~550자)으로 작성하세요.
다른 기사들은 기사당 6~8문장(300~400자)으로, "지금 ~에 관심이 있다면", "~를 고민 중이라면" 같은
도입부로 시작해 기사 내용을 구체적으로 풀어내세요.
마지막 한두 문장은 기사에서 얻을 수 있는 것을 구체적으로 언급하세요. 각 기사 소개는 "●" 로 시작하세요.
...
총 8~10문장(400~500자)으로, 어제 들은 음악의 분위기를 먼저 세 문장 정도로 묘사한 뒤
추천 곡 2개를 각각 두세 문장씩 소개하세요.
추천 곡은 "●곡명 / 아티스트" 형식으로 표시하고, 각 곡마다 추천 이유와 어울리는 상황을 설명하세요.
...
총 8~10문장(400~500자)으로, "어제의 사운드트랙은 ~였다" 식의 표현으로 시작해 서너 문장에 걸쳐
그 감정의 흐름을 그린 뒤, 오늘 추천 곡 2개를 "●곡명 / 아티스트 — 설명" 형식으로
각각 두세 문장씩 이어서 소개하세요.
...
추천 곡 2: {rec_2}"""
    ...
    # variant C: 추천 곡 1 심층 소개 + 추천 곡 2 짧게 언급
    # (기존 버전은 "1곡만 소개"라 지시하면서 rec_2도 넘겨 모순이 있었음 → 역할 분리로 수정)
    _wrap(
        """다음 음악 데이터를 바탕으로 어제 청취 감상과 오늘 추천 이유를 자연스럽게 서술하세요.
먼저 어제 들은 음악의 분위기를 두세 문장으로 묘사하세요.
그 다음 추천 곡 1을 "●곡명 / 아티스트" 형식으로 제시하고, 그 곡이나 앨범, 아티스트에 대한
이야기를 담아 5~6문장으로 깊이 있게 소개하세요.
마지막으로 추천 곡 2는 "●곡명 / 아티스트 — 한 줄 설명" 형식으로 짧게 한 문장만 덧붙이세요.
전체 분량은 8~10문장(400~500자)이 되도록 하세요.
...
def generate_music_section(music_data: dict, mood_summary: dict) -> tuple[str, int | None]:
    ...
    return _call_llm(prompt, max_tokens=1000) or f"어제 무드: {mood_ko}\n추천 곡: {rec_1}, {rec_2}", idx
