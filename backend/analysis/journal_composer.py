...
- A5 양면 기준: 각 섹션 글자수 충분히 확보
    ...
    · 기사소개: 메인 24~28문장(1300~1600자) / 서브 10~12문장(500~700자)
...
- 기사 소개/음악 섹션은 여러 항목(기사 4개 / 어제청취+추천곡2개)을 한 번의 LLM 호출로
...
[아래 구조를 반드시 따를 것 — 각 단계의 문장수는 위 [분량] 지시에 맞게 비율껏 늘릴 것]
1) 기사의 핵심 사실을 짧게 소개 (전체 분량의 10~15%)
2) 기사 내용을 구체적으로 풀어 설명 (배경, 세부 내용, 왜 중요한지, 관련 맥락 등 — 요약이 아니라 살을 붙여서.
   [분량] 지시를 채우려면 이 부분이 여러 문단에 걸쳐 전체 분량의 70~80%를 차지해야 함)
3) 사용자의 관심사와 "자연스럽게" 연결되는 지점이 있을 때만 짧게 짚기 (전체 분량의 10~15%)

[주의]
- 사용자의 핵심 관심사·유튜브 시청 이력·키워드를 억지로 연결하지 마세요.
  실제로 관련 있을 때만 자연스럽게 언급하고, 관련 없으면 기사 자체의 매력과 정보만으로 소개하세요.
- 어조는 친근하되 정보 밀도가 높아야 합니다.
- 기사 제목은 지면에 별도로 이미 표시되므로, 본문에서 제목을 그대로 반복하거나
  "기사 제목:", "기사 요약:" 같은 입력 데이터의 라벨을 절대 출력하지 마세요.
  본문은 곧바로 핵심 사실을 설명하는 자연스러운 문장으로 시작하세요.

아래는 참고용 입력 데이터이며, 이 라벨과 형식을 그대로 출력에 옮기지 마세요.
...
[아래 구조를 반드시 따를 것 — 각 단계의 문장수는 위 [분량] 지시에 맞게 비율껏 늘릴 것]
1) "지금 ~에 관심이 있다면", "~를 고민 중이라면" 같은 도입부 (전체 분량의 10~15%)
2) 기사 내용을 구체적으로 풀어 설명 (배경, 세부 내용, 핵심 주장 등 — 요약이 아니라 살을 붙여서.
   [분량] 지시를 채우려면 이 부분이 여러 문단에 걸쳐 전체 분량의 70~80%를 차지해야 함)
3) 기사에서 얻을 수 있는 것을 구체적으로 언급하는 마무리 (전체 분량의 10~15%)
...
_MAIN_ARTICLE_LENGTH = (
    "이 기사는 오늘의 메인 기사입니다. 지면에서 헤드라인 박스와 '이어서' 박스, 두 곳에 나눠 실리므로 "
    "24~28문장(1300~1600자)으로 충분히 길게 작성하세요. 절대 16문장 이하로 줄이지 마세요. "
    "글 중반에 자연스러운 문단 전환이 한두 번 있으면 좋습니다(글이 뚝 잘려도 다음 문단으로 이어지도록)."
)
_SUB_ARTICLE_LENGTH = "이 기사는 서브 기사입니다. 10~12문장(500~700자)으로 작성하세요. 절대 6문장 이하로 줄이지 마세요."
...
def generate_article_intros(
    ...
    """추천 기사 4개를 기사당 1회씩 개별 LLM 호출로 소개문 생성.

    반환: ([{"title", "link", "intro", "is_main"}, ...], variant_idx_list).
    기사마다 variant(어조)를 독립적으로 랜덤 선택해, 같은 날 안에서도 기사별로 톤이 섞일 수 있다.
    """
    articles = articles[:4]
    if not articles:
        return [], []
    ...
    for i, article in enumerate(articles):
        ...
        max_tokens = 2800 if is_main else 1300
    ...
    return results, variant_indices
...
def run_journal_composition(
    ...
    article_intros, article_idxs = generate_article_intros(
