...
_MAIN_ARTICLE_LENGTH = (
    ...
    "16~20문장(900~1100자)으로 충분히 길게 작성하세요. 절대 10문장 이하로 줄이지 마세요. "
    "글 중반에 자연스러운 문단 전환이 한 번 있으면 좋습니다(글이 뚝 잘려도 다음 문단으로 이어지도록)."
)
...
def generate_article_intros(
    ...
    같은 variant(어조)를 기사 4개 모두에 적용해 하루치 톤을 통일한다.
    """
    articles = articles[:4]
    if not articles:
        return [], None
    ...
    for i, article in enumerate(articles):
        ...
        max_tokens = 2000 if is_main else 900
