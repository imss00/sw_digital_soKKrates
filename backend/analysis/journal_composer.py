...
# ────────────────────────────────────────────────────────────────
# B-4. 어제 회고
# (본문 섹션 — 8~10문장, 400~550자. 사용자 감정 단정 금지 규칙은 _OUTPUT_RULES에 공통 적용됨)
# ────────────────────────────────────────────────────────────────

_REFLECTION_VARIANTS = [
    # variant A: 시간 흐름 서술 — 아침→저녁 (사실 + 장면 묘사)
    _wrap(
        """다음 데이터를 바탕으로 어제 하루 있었던 일을 8~10문장(400~550자)으로 정리해주세요.
시간 흐름(아침→낮→저녁)을 따라가며, "~했다" 체로 서술하세요.
활동과 사실을 중심에 두되, 장면이나 분위기를 생생하게 그려내는 묘사·비유는 자유롭게 곁들이세요.
평가나 조언 없이, 구체적인 키워드나 활동을 최소 4개 포함하세요.
...
어제의 장면: {photo_narrative}"""
    ...
    # variant B: 키워드 중심 압축 서술 (사실 + 비유)
    _wrap(
        """다음 데이터를 바탕으로 어제 하루를 8~10문장(400~550자)으로 정리해주세요.
하루를 관통한 키워드 2~3개를 각각 한두 문장씩 풀어 쓰며, 무엇에 시간을 썼는지를
"~을 들었다", "~을 봤다", "~을 찾아봤다" 같은 서술과 함께, 그 활동이나 분위기를
비유적으로 그려내는 문장을 한두 개 곁들이세요 (사용자가 그렇게 느꼈다고 단정하지는 마세요).
마지막 한두 문장은 하루 전체를 압축하는 묘사로 마무리하세요.
...
어제의 장면: {photo_narrative}"""
    ...
    # variant C: 감각(청각/시각/관심사) 중심 묘사
    _wrap(
        """다음 데이터를 바탕으로 어제 하루를 8~10문장(400~550자)으로 정리해주세요.
청각(음악), 시각(영상/사진), 관심사(검색/콘텐츠)라는 세 영역을 각각 두세 문장씩 담아,
무엇을 들었고 무엇을 봤고 무엇에 관심을 두었는지 그려내세요.
"~을 들었다", "~을 봤다", "~을 접했다" 같은 서술에 장면 묘사나 비유를 자연스럽게 곁들이되,
"~를 느끼며", "~를 생각하며" 처럼 사용자의 감정을 단정하는 연결어는 쓰지 마세요.
