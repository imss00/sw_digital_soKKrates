...
## 팀원 온보딩 (처음 시작할 때 여기부터)

### Step 1. 앱 오너(수빈)에게 이메일 전달
Google + Spotify 테스트 계정 등록이 필요합니다. 본인 Google 이메일을 수빈에게 알려주세요.  
수빈이 등록해주면 Step 2로 넘어갑니다.

### Step 2. 내 계정 만들기 (Google 로그인)
브라우저에서 아래 URL 접속 → 반환된 `auth_url` 클릭 → Google 계정으로 로그인:
```
https://swdigitalsokkrates-production.up.railway.app/auth/google
```
로그인 완료 후 응답에서 **`user_id` 숫자를 메모해두세요.** 이후 모든 API 호출에 씁니다.

### Step 3. 역할별 다음 단계

**Phase 2-3 (AI 분석) 담당이라면**
- [Phase 2-3 개발 가이드](#phase-2-3-개발-가이드-팀원용) 섹션으로 이동
- `backend/analysis/` 파일 열고 TODO 채우면 됩니다
- 로컬 설치 없이 Railway DB에 직접 연결 가능 (`DATABASE_URL`만 수빈에게 받으세요)

**Chrome Extension 테스트가 필요하다면**
1. `chrome://extensions` → 개발자 모드 ON → `chrome-extension/` 폴더 로드
2. Extension 팝업 열기 → **API URL**: `https://swdigitalsokkrates-production.up.railway.app`
3. Extension 팝업 → **User ID**: Step 2에서 받은 숫자 입력
4. 이제 브라우징 기록이 자동으로 내 계정으로 수집됩니다

**Spotify 연동이 필요하다면**
수빈이 Spotify 앱에 이메일 추가한 뒤:
```
https://swdigitalsokkrates-production.up.railway.app/auth/spotify
```
접속 → 반환된 `auth_url` 클릭 → Spotify 로그인
...
팀원이 테스트 요청하면 아래 두 곳에 이메일 추가:

- **Google**: [Google Cloud Console](https://console.cloud.google.com) → API 및 서비스 → OAuth 동의 화면 → 테스트 사용자 → 이메일 추가
- **Spotify**: [Spotify Developer Dashboard](https://developer.spotify.com/dashboard) → 앱 선택 → Settings → User Management → 이메일 추가 (최대 25명)
